    ...
    Admin.findOne({ username: adminData.username }).then((admin, err) => {
        if (admin) {
            if (adminData.password === admin.password) {
                res.send({ message: 'Login Successfull', id: admin.id })
            }
            else {
                res.send({ message: 'Wrong Password' });
            }
...
const addToCart = (req, res) => {
    try {
        Cart.findOne({ item_name: req.body.item_name, user_id: req.params.id }).then((item, err) => {
            if (item) {
                res.send({ message: 'Already Added' });
            }
            else {
                const cart = new Cart(req.body);
                cart.save().then((item, err) => {
                    if (item) {
                        res.send({ message: "Added to Cart" });
                    }
                    else {
                        res.send({ message: "Error while adding to Cart" })
                    }
                })
            }
        })
...
const fetchCartData = async (req, res) => {
    const cartData = await Cart.find({ user_id: req.params.id });
    if (cartData) {
        res.send(cartData)
    }
    else {
        console.log('Nothing Found in Cart Schema');
    }
}

const deleteCart = async (req, res) => {
    const item = await Cart.deleteOne({ user_id: req.params.id, item_name: req.body.item_name });
    if (item) {
        res.send({ message: "Item Deleted from Cart" });
    }
    else {
        res.send({ message: "Error while deleting from Cart" });
    }
}

const buyNow = (req, res) => {
    const order = {
        item_name: req.body.order.item_name, price: req.body.order.price, item_type: req.body.order.item_type,
        user_id: req.params.id, status: 'pending'
    };
    try {
...
const myOrder = async (req, res) => {
    try {
        const orderData = await Order.find({ user_id: req.params.id });
        if (orderData) {
            res.send(orderData);
        }
        else {
            console.log('Error while fetching order Data of ' + currentUser);
        }
    } catch (err) {
        console.log(err);
    }
...
const deleteMyOrder = async (req, res) => {
    try {
        ...
        await Order.deleteOne({ _id: req.body.id }).then((order, err) => {
            if (order) {
